...
func BindAndValid(c *gin.Context, form interface{}) (int, int, string) {
	...
	errMsg := ""
	if err != nil {
		return http.StatusBadRequest, constant.INVALID_PARAMS, errMsg
	}

	valid := validation.Validation{}
	check, err := valid.Valid(form)
	if err != nil {
		return http.StatusInternalServerError, constant.ERROR, err.Error()
	}
	if !check {
		err = MarkErrors(valid.Errors)
		return http.StatusBadRequest, constant.INVALID_PARAMS, err.Error()
	}
